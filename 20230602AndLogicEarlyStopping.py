...
model_and.fit(x_train, y_train, epochs=10000)

# 모델 훈련
x_val = [[0,0],[0,1],[1,0],[1,1]]
y_val = [[9.9e-100],[9.9e-100],[9.9e-100],[1.0000000e+00]]

# 검증 손실(val_loss)이 10번 연속으로 개선되지 않으면 학습을 중지합니다.
early_stopping = EarlyStopping(monitor='val_loss', patience=10)
model_and.fit(x_train, y_train, epochs=10000, validation_data=(x_val, y_val), callbacks=[early_stopping])

# 학습이 중지된 이후에도 가장 좋은 모델의 가중치를 저장합니다.
model_and.save_weights('best_model.h5')
best_model = model_and.save_weights('best_model.h5')
print(best_model)
